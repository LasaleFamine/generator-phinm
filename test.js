...
test.beforeEach(async () => {
	...
	generator = helpers.createGenerator('phinm:app', ['../app'], null, {skipInstall: true});
});
...
test.serial('Appveyor option', async () => {
	helpers.mockPrompt(generator, {
		moduleName: 'test',
		githubUsername: 'test',
		website: 'test.com',
...
test.serial('codecov option', async () => {
	...
	assert.fileContent('.gitignore', /coverage/);
	assert.fileContent('package.json', /"xo && nyc ava"/);
	assert.fileContent('package.json', /"test:travis":/);
	assert.fileContent('package.json', /nyc report --reporter=text-lcov > coverage.lcov && codecov"/);
	assert.fileContent('package.json', /"nyc": "/);
	assert.fileContent('package.json', /"codecov":/);
	assert.fileContent('package.json', /"lcov"/);
	assert.fileContent('.travis.yml', /yarn test:travis/);
});
